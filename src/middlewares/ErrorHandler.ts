...
const errorHandler = (err, req, res, next) => { // eslint-disable-line
  ...
  // Otherwise try to identify the type of error (mongoose validation,
  // mongodb unique, ...)
  // If we can't identify it, respond with a generic 500 error
  let responseErr = err instanceof CustomError ? err : null

  // Handle errors created with 'http-errors' or similar that have a
  ...
  // headers: omit(req.headers, ['x-api-key', 'cookie', 'password',
  // 'confirmPassword']),
  ...
  // In some occasions like when invalid JSON is supplied `res.respond` might be
  // not yet available,
